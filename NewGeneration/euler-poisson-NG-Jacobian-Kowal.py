...
def Df(s, var):
    ...
    g2 = var[4]
    g3 = var[5]
    return [[0, -(C-B)/A*r, -(C-B)/A*q, 0, c/A, -b/A],
         [-(A-C)/B*r, 0, -(A-C)/B*p, -c/B,0,a/B],
         [-(B-A)/C*q, -(B-A)/C*p, 0, b/C, -a/C, 0],
         [0,0,0,0,r,-q],
         [0,0,0,-r,0,p],
         [0,0,0,q,-p,0]]
...
hh = ode(f, Df).set_integrator('dop853')
hh.set_initial_value(init, t0)
t1 = TotalTime
...
g2_slt = []
g3_slt = []
while hh.successful() and hh.t <= TotalTime:
    aaa = hh.integrate(hh.t+dt)
    p_slt.append(aaa[0])
    q_slt.append(aaa[1])
    r_slt.append(aaa[2])
    g1_slt.append(aaa[3])
